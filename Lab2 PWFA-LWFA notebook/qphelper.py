...
def yujian_action_linear(inputDeckTemplateName,
    ...
    print("output filename: ",inputDeckTemplateName)
    with open(inputDeckTemplateName,'w') as outfile:
        json.dump(inputDeck,outfile,indent=4)
    Lambda = peak_density_driver*sigma_r_driver**2    
    print('Lambda = ',Lambda)
    if Lambda > 1:
        print("Nonlinear Regime!")
    elif Lambda <= 1 and peak_density_driver <= 0.9: 
        print("Linear/Quasi-Linear Regime!")
    
def makeWidgetsForPWFAInput(regime):        
    style = {'description_width': '350px'}
    layout = Layout(width='55%')
    if regime == 'nonlinear':
        npeak = 5
        fname = 'qpinput_nonlinear.json'
    if regime == 'linear':
        npeak = 0.11
        fname = 'qpinput_linear.json'

    inputDeckTemplateNameW = widgets.Text(value=fname, description='Template Input File:',style=style,layout=layout)

    indxW = widgets.IntText(value=8, description='indx:', style=style, layout=layout)
    indyW = widgets.IntText(value=8, description='indy:', style=style, layout=layout)
    indzW = widgets.IntText(value=8, description='indz:', style=style, layout=layout)
    dt = widgets.IntText(value=10.0, description='dt:', style=style, layout=layout)
    ...
    peak_density_driverW = widgets.FloatText(value = npeak, description='peak density (driver):', style=style, layout=layout)

    
    interact_calc(yujian_action_linear,inputDeckTemplateName = inputDeckTemplateNameW,
                  indx = indxW,indy=indyW,indz=indzW,dt = dt,
                  n0 = n0W,boxXlength=boxXlengthW,boxYlength=boxYlengthW,boxZlength=boxZlengthW,
                  z_driver = z_driverW,
...
def makeplot(rundir,fname,isLinear):
    ...
    def plot1_qp(x_position):
        ...
        if(isLinear): 
            #cs1 = plt.pcolormesh(xi,x,da,cmap=colormap)
             cs1 = plt.pcolormesh(xi,x,da,vmin=-1.5,vmax=datamax,cmap='Blues_r')
        else:
            #cs1 = plt.pcolormesh(xi,x,da,vmin=datamin,vmax=datamax,cmap=colormap)
            cs1 = plt.pcolormesh(xi,x,da,vmin=-1.5,vmax=datamax,cmap='Blues')
        ...
        if(isLinear):
            ...
            ax2.plot(xin,np.flip([rhoth(i,x_position) for i in xin-xi0]),'b--',label='Numerical Integration')
            ax2.legend()                     
