...
def plot_data(dirname,off=0.0,theory=True,xlim_max=None):
	...
	if(first_out > 0):
		plt.plot(np.arange(int(xi[-1]+2)), np.ones(int(xi[-1]+2))*pr_solution,'r--',label=r'Theory Eq. (36)' )
		if(use_student_solution):
			plt.plot(np.arange(int(xi[-1]+2)), np.ones(int(xi[-1]+2))*student_solution,'b--',label=r'Student Solution' )
	else:
		plt.plot(np.arange(int(xi[-1]+2)), np.ones(int(xi[-1]+2))*pr_solution,'r--',label=r'Theory Eq. (36)' )
		if(use_student_solution):
			plt.plot(np.arange(int(xi[-1]+2)), np.ones(int(xi[-1]+2))*student_solution,'b--',label=r'Student Solution' )
