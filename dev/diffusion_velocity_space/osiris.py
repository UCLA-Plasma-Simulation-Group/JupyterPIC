...
from ipywidgets import interact, fixed, interact_manual, Layout, FloatSlider, IntSlider
...
def run_upic_es(rundir='',inputfile='pinput2',np=None):
    ...
    if os.path.isfile('upic-es.out'):
        localexec = 'upic-es.out'
    else:
        localexec = False
    ...
    shutil.copyfile(inputfile,rundir+'/pinput2')
    if localexec:
        shutil.copyfile(localexec,rundir+'/upic-es.out')
...
def run_upic_es_subtech(rundir='',inputfile='pinput2',np=None):
    ...
    def combine_h5_2d_diff(path, ex):
        in_file = '../' + rundir + '/' + path + '/' + ex + '/'
        in_file2 = '../' + rundir + '-nodriv/' + path + '/' + ex + '/'
        out_file = ex + 'subs.h5'
        for path in execute(["python", "../combine_h5_util_2d_diff.py", in_file, in_file2, out_file]):
            IPython.display.clear_output(wait=True)
            print(path, end='')
    ...
    if(not os.path.isdir(rundir)):
       os.mkdir(rundir)
        ...
        shutil.rmtree(rundir)
        os.mkdir(rundir)
    if(not os.path.isdir(rundir+'-nodriv')):
       os.mkdir(rundir+'-nodriv')
    else:
        shutil.rmtree(rundir+'-nodriv')
        os.mkdir(rundir+'-nodriv')

    shutil.copyfile(inputfile,rundir+'/pinput2')
    shutil.copyfile(inputfile,rundir+'-nodriv/pinput2')
    if localexec:
        shutil.copyfile(localexec,rundir+'/upic-es.out')
        shutil.copyfile(localexec,rundir+'-nodriv/upic-es.out')

    with open(rundir+'-nodriv/pinput2') as osdata:
    ...
    with open(rundir+'-nodriv/pinput2','w') as f:
        for line in data:
            f.write(line)
    ...
    os.chdir(rundir+'-nodriv')
    
    print(os.getcwd())
    #time.sleep(100)

    # run the upic-es executable
    print('running upic-es.out without driver...')
    if np==None:
        if localexec:
            localexec = './'+localexec
            for path in execute([localexec]):
    ...
    combine_h5_2d('DIAG', 'Ey')
    combine_h5_2d('DIAG', 'pot')
    combine_h5_2d_diff('DIAG', 'Ex')
    combine_h5_2d_diff('DIAG', 'pot')
    print('combine_h5_2d completed normally')
...
#            print(path, end='')
    ...
    def combine_h5_test_charges():
        in_file = rundir + '/MS/PHA/p1x1/tests//'
        out_file = rundir + '/test-charges.h5'
        for path in execute(["python", "/usr/local/osiris/combine_h5_util_2d_x2.py", in_file, out_file]):
            if print_out == 'yes':
                IPython.display.clear_output(wait=True)
#            print(path, end='')
    if os.path.isfile('osiris-1D.e'):
    ...
    if (os.path.isdir(rundir+'/MS/DENSITY/ions/charge')):
        ...
        if combine == 'yes':
            combine_h5_iaw_1d()
    if (os.path.isdir(rundir+'/MS/PHA/p1x1/tests')):
        if print_out == 'yes':
            print('combining PHASE space files for drag+diffusion')
            # combine_h5_test_charges()
...
def phaseinteract_upic(rundir='',
   ...
   def fu(n=0):
       ...
       im = ax[1].imshow(np.log(np.abs(data[n][0]+0.001)),cmap='OrRd',origin='lower',
              extent=[xaxis[0], xaxis[1], yaxis[0], yaxis[1]],
              aspect='auto')
       #cax = fig.add_axes([0.27, 0.8, 0.5, 0.05])
       fig.colorbar(im, cax=cax, orientation='vertical')
       #plt.colorbar()
       ax[1].set_title('time = '+str(data[n][1]))
       if(xlim != [-1,-1]):
           ax[1].set_xlim(xlim)
       if(ylim != [-1,-1]):
           ax[1].set_ylim(ylim)
       ...
       ax[1].set_ylabel('$p_x/m_ev_{the}$')
       xrange_dist = np.linspace(yaxis[0],yaxis[1],len(np.log(np.sum(np.abs(data[n][0]+0.001),axis=1))))
       ax[0].plot(xrange_dist,np.log(np.sum(np.abs(data[n][0]+0.001),axis=1)))
       #ax[0].set_xlim(xrange_dist,[yaxis[1],yaxis[0]])
       ax[0].set_xlabel('$v/v_{the}$')
       ax[0].set_ylabel('$f(v)$')
       fig.tight_layout()
       return fig

   control = widgets.IntSlider(value=0,min=0,max=len(data)-1,description='iteration')
   plotwidget = widgets.interactive_output(fu, {
                       'n':control});
   play = widgets.Play(
        value=0,
        min=0,
   ...
   widgets.jslink((play, 'value'), (control, 'value'))
   display(widgets.VBox([plotwidget, control, play]))


def landau_quad(rundir='',
   xlim=[-1,-1],ylim=[-1,-1],zlim=[-1,-1],
   plotdata=[]):
   ...
   odir = os.path.join(workdir.replace('-nodriv',''), 'DIAG/Vx_x')
   files = sorted(os.listdir(odir))
   ...
   odirpot = os.path.join(workdir, 'potsubs.h5')
   fherepot = h5py.File(odirpot, 'r')
   datapot = fherepot['Data']
   xaxispot = fherepot['AXIS/AXIS1'][:]

   odirexsq = os.path.join(workdir, 'Exsubs.h5')
   fhereexsq = h5py.File(odirexsq, 'r')
   dataexsq = np.sum(fhereexsq['Data'][:]*fhereexsq['Data'][:],axis=1)
   xaxisexsq = np.arange(fhereexsq['AXIS/AXIS2'][0],fhereexsq['AXIS/AXIS2'][1],fhereexsq.attrs['DT'])
   #xaxisexsq = fhereexsq['AXIS/AXIS1'][:]

   def fu(n=0):
       fig,ax = plt.subplots(2,2,figsize=(12, 8))
       divider = make_axes_locatable(ax[1,1])
       cax = divider.append_axes('right', size='5%', pad=0.05)
       im = ax[1,1].imshow(np.log(np.abs(data[n][0]+0.001)),cmap='OrRd',origin='lower',
              extent=[xaxis[0], xaxis[1], yaxis[0], yaxis[1]],
              aspect='auto')
       #cax = fig.add_axes([0.27, 0.8, 0.5, 0.05])
       fig.colorbar(im, cax=cax, orientation='vertical')
       #plt.colorbar()
       ax[1,1].set_title('time = '+str(data[n][1]))
       if(xlim != [-1,-1]):
           ax[1,1].set_xlim(xlim)
       if(ylim != [-1,-1]):
           ax[1,1].set_ylim(ylim)
       if(zlim != [-1,-1]):
           ax[1,1].set_clim(zlim)
       ax[1,1].set_xlabel('$x/\lambda_D$')
       ax[1,1].set_ylabel('$p_x/m_ev_{the}$')
       xrange_dist = np.linspace(yaxis[0],yaxis[1],len(np.log(np.sum(np.abs(data[n][0]+0.001),axis=1))))
       ax[1,0].plot(xrange_dist,np.log(np.sum(np.abs(data[n][0]+0.001),axis=1)))
       #ax[0].set_xlim(xrange_dist,[yaxis[1],yaxis[0]])
       ax[1,0].set_xlabel('$v/v_{the}$')
       ax[1,0].set_ylabel('$f(v)$')
       ax[1,0].set_title('time = '+str(data[n][1]))
       
       ax[0,0].semilogy(dataexsq+1e-12)
       ax[0,0].set_xlabel('time [$\omega_p^{-1}$]')
       ax[0,0].set_ylabel('$<E_x^2>_{total}$')
       esqymax = np.max(dataexsq)*2
       esqymin = np.min(dataexsq[int(0.1*len(dataexsq)):])/2
       ax[0,0].set_ylim([esqymin,esqymax])
       xrange_pot = np.linspace(xaxispot[0],xaxispot[1],len(datapot[n]))
       ax[0,1].plot(xrange_pot,datapot[n][:])
       ax[0,1].set_xlabel('$x/\lambda_D$')
       ax[0,1].set_ylabel('$e\Phi/m_ev_{the}^2$')
       ax[0,1].set_title('time = '+str(data[n][1]))
...
## for widgets
# import glob
# from ipywidgets import interact_manual,fixed,Layout,interact, FloatSlider,IntSlider
# ##
...
def landau_damping_deck_maker(iname='ldi-ions.txt', oname='case1.txt', amp=0.3,
             wavew = 1.03, wavemode=6, drivdur = 2.5, ppc = 100, tend=100):
    ...
    for i in range(len(data)):
        ...
        if 'TIMEFLAT' in data[i]:
            data[i] = ' TIMEFLAT = '+str(drivdur)+',\n'
        if 'NPX' in data[i]:
            data[i]=' NPX = '+str(ppc*256)+',\n'
        if 'TEND' in data[i]:
...
#     osiris.run_upic_es(rundir=dirname,inputfile=oname)
    run_upic_es_subtech(rundir=dirname,inputfile=oname)
 ...
 #
    ...
    #ax[0] = 
    plot_tk_arb(outdirname+'-nodriv','potsubs',klim=0.5,tlim=tend)
#     plot_tk_ldi(outdirname,'Ex',modeno=wavemode,tlim=tend)
    #ax[1] = 
    plot_tk_ldi(outdirname+'-nodriv','Exsubs',modeno=wavemode,tlim=tend)
#     phaseinteract_upic(rundir=dirname)
    landau_quad(rundir=outdirname+'-nodriv')
...
def landau_damping_widget():
    ...
    d = widgets.FloatText(value=1.1,description='Driver Frequency (in omega_pe):',style=style,layout=layout)
    e = widgets.IntSlider(value=13,min=1,max=22,step=1,description='Driver Mode No. (fundamental modes of box):',
                          style=style,layout=layout)
    drivdur = widgets.FloatText(value=2.5,description='Driver Duration (in omega_pe):',style=style,layout=layout)
    f = widgets.IntText(value=100,description='Particles Per Cell (PPC):',style=style,layout=layout)
    g = widgets.FloatText(value=100.0,description='TEND or the Total Simulation Time:',style=style,layout=layout)
  

    im = interact_calc(landau_damping_deck_maker, iname=a,oname=b,amp=c,wavew=d, wavemode = e, drivdur = drivdur, ppc = f,tend=g);
